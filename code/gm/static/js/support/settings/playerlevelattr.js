...
function setwateredit(layerid) {
    ...
    if (gpRebate == "" || gpRebate == "undefined" || gpRebate == null || gpRebate.length == 0) {
        ...
        $.each(validGames, function(code, name){
            attrJsonData[groupid][layerid].rebateSetting[code] = {floatRate: {},fixRate:0};
        })
    ...
    var html = "";
    $.each(validGames, function(code, name){
        html += "<tr gpid=\"" + code + "\" layerid=\"" + layerid + "\">";
        html += "<td>" + code + "</td>";
        html += "<td gpname=gpname>" + name + "</td>";
        html += '<td><input name="rrate" onkeyup="setRateVal(this, 100)" class="form-control" type="text" value="' + gpRebate[code].fixRate + '"></td>';

        var fstr = new Array();
        for (var nd in gpRebate[code].floatRate) {
            var _fstr = gpRebate[code].floatRate[nd].amount + "_" + gpRebate[code].floatRate[nd].rate;
            fstr.push(_fstr);
        }
        html += '<td name="stepped">' + fstr.join("|") + '</td>';
        html += '<td><a href="#waterLeverModal" onclick="setuplevertr(\'' + code + '\', \'' + name + '\', \'' + groupname + '\');" data-toggle="modal" class="btn btn-xs blue">返水设置</a></td></tr>';
...
function setuplevertr(gpid, gpname, gamename) {
    ...
    if (floatRate == "" || floatRate == undefined || floatRate == null || floatRate.length == 0) {
        ...
        for (nrate in floatRate) {
            addlevertr(floatRate[nrate].amount, floatRate[nrate].rate);
        }
    }
}
