    ...
    $.fn.Carouselone = function (options) {
        var defaults = { "background": "red", "play": "true", "auto": "true", "prevButton": "", "nextButton": "", "AnTimer": "800", "playTimer": "5000", "WidthMax": "false", "fade": "false", "eventClick": "false" }; var str = $.extend({}, defaults, options); this.each(function () {
            ...
            oThis.each(function () {
                ...
                var oItem = oThis.find(".pj-Carousel-item"); oCarouseL.css("width", oboxWidth * oItemSzie + "px"); var color = ""; for (var i = 0; i < oItemSzie; i++) { color += "<div class ='pj-Carousel-color'><span></span><strong><i></i><i></i><i></i><i></i></strong></div>" } oActive.append(color); var colorBg = oThis.find(".pj-Carousel-color").css("background");
                oThis.find(".pj-Carousel-color").eq(0).addClass("on"); oItem.css("width", oboxWidth); if (str.eventClick == "true") { oThis.find(".pj-Carousel-color").bind("click", Toevevt) } else { oThis.find(".pj-Carousel-color").bind("mouseover", Toevevt) }
                function Toevevt() { oThis.find(".pj-Carousel-color strong i").removeClass("on");var oIndex = $(this).index(); $(this).addClass("on").siblings(".pj-Carousel-color").removeClass("on"); if (str.fade == "true") { oThis.find(".pj-Carousel-item").eq(oIndex).fadeIn(str.WidthMax).siblings(".pj-Carousel-item").fadeOut(str.WidthMax) } else { oCarouseL.animate({ "left": -oboxWidth * oIndex + "px" }, str.AnTimer) } num = oIndex } var num = 0; var timer = null; if (str.fade == "true") { oItem.css({ "clear": "both", "position": "absolute", "left": "0", "top": "0" }); oItem.eq(0).show().siblings(".pj-Carousel-item").hide() } if (str.auto == "true") {
                    timer = setInterval(plays, str.playTimer);
                    var i = 4;
                    var playnum = 0;
                    countTime = setInterval(function () {
                        i = --i < 0 ? 0 : i;
                        oThis.find(".pj-Carousel-color strong i").eq(playnum).addClass("on")
                        if (i == 0) {
                    ...
                    oThis.find(".pj-Carousel-color strong i").removeClass("on")
                    var i = 4;
                    var playnum = 0;
                    countTime = setInterval(function () {
                        i = --i < 0 ? 0 : i;
                        $('.pj-Carousel-color.on').find("strong i").eq(playnum).addClass("on")
                        if (i == 0) {
                            clearInterval(countTime);
                        }
                        playnum++;
                    ...
                    oThis.find(".pj-Carousel-color").eq(num).addClass("on").siblings(".pj-Carousel-color").removeClass("on");
                    if (str.fade == "true") { oThis.find(".pj-Carousel-item").eq(num).fadeIn(str.AnTimer).siblings(".pj-Carousel-item").fadeOut(str.AnTimer) } else { oCarouseL.animate({ "left": -oboxWidth * num + "px" }, str.AnTimer) }
                } if (str.prevButton != "") {
                    var oPrev = str.prevButton; $(oThis.find(oPrev)).bind("click", function () {
                ...
                } if (str.nextButton != "") { var oNext = str.nextButton; $(oThis.find(oNext)).bind("click", function () { oCarouseL.stop(true, true); if (str.play == "true") { num++; if (num >= oItemSzie) { num = 0 } publicFn() } if (str.play == "false") { if (num >= 0 && num < oItemSzie - 1) { num++; publicFn() } } }) } oThis.find(".pj-Carousel").bind("mouseover", function () { clearInterval(timer) }); oThis.find(".pj-Carousel").bind("mouseout", function () { timer = setInterval(plays, str.playTimer) })
            })
        }); return this
    }
