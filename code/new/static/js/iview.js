 ...
 */ (function ($, window, undefined) {
	...
	var iView = function (el, options) {
           ...
           //  iv.slider.css({ 'position': 'relative', 'width': '1920px'});
			//Necessary variables.
			iv.defs = {
				slide: 0,
				total: 0,
			...
			//Set Timer Element
			iv.sliderContent.append('<div class="iview-time-box"><div id="iview-timer"><span></span></div></div>');
		...
		//Find captions
			...
			iv.sliderContent.bind('iView:goSlide', function (event, slide) {
				...
				iv.cleanTimer();
                iv.slider.css('background', 'url("' + iv.defs.image.data('iview:image') + '") no-repeat center top');
	...
	iView.prototype = {
		...
		startSlider: function () {
			...
			iv.iviewTimer.append("<span></span>");
			...
			if (iv.options.controlNav) {
				...
				if (!iv.options.controlNavThumbs && iv.options.controlNavTooltip) iviewControl += '<div id="iview-tooltip"><div class="holder"><div class="containeron">' + iviewTooltip + '</div></div></div>';
		...
		addStrips: function (vertical, opts) {
			...
			for (var i = 0; i < opts.strips; i++) {
				...
				var strip = $('<div class="iview-strip"></div>').css({
					...
					top: top,
					left: left,
                    background: 'url("' + iv.defs.image.data('iview:image') + '") no-repeat center top',
					opacity: 0
				});
					...
					for (var rows = 0; rows < iv.options.blockRows; rows++) {
						if (Col >= 0 && Col < iv.options.blockCols) {
							(function () {
								...
								var w = block.width();
								var h = block.height();
                                if (fx == 'block-expand' || fx == 'slice-top-fade-reverse') {
