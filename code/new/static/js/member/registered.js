            ...
            $.ajax({
                ...
                success: function (datas) {
                    if (datas.code == 200) {
                        ...
                        captchaObj.reset();
                        window.location.href = '/zh-cn/member/Registeredok';
                    } else if (datas.code == 1) {
                        swal({ title: "", text: datas.Message, type: "warning" });
                        captchaObj.reset();
                    } else if (datas.code == 500) {
                        Singletips("user_Error", datas.Message);
                        $("#Password").val("");
                        $("#Passwordcof").val("");
                        captchaObj.reset();
                    }
...
$.ajax({
    ...
    url: "/zh-cn/publicView/getcaptcha?t=" + (new Date()).getTime(), // 加随机数防止缓存
    type: "get",
    dataType: "json",
