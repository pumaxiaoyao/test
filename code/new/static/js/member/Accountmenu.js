$(function () {
    ...
    $("#refreshMainBt").click(function () {
        ...
        $.post("/API/Common/RefreshBalance", { partnerCode: 'MAIN', clienttype: 4 }, function (response) {
    ...
    $("#refreshAgentBt").click(function () {
        $("#MainBalance_b").html('<img src="/static/img/loading.gif" />');
        $.post("/agent/agents/RefreshBalance", { partnerCode: 'MAIN', clienttype: 4 }, function (response) {
            response = JSON.parse(response);
            if (response.code == 200){
                console.log(response.data);
                var balance = parseFloat(response.data).toFixed(2);
                $("#MainBalance_b").html(balance);
                $("#MainBalance_Nav").html(balance);
