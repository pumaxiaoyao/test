...
//////////////////////////////////// 异步通知验签  ////////////////////////////////////////////////
		...
		System.out.println("速汇宝返回的签名参数排序：" + signInfo.length() + " -->" + signInfo);		
		System.out.println("速汇宝返回的签名：" + dinpaySign.length() + " -->" + dinpaySign);								
		boolean result = false;
		
		if("RSA-S".equals(sign_type)){ // sign_type = "RSA-S"			
			
			/**
			1)dinpay_public_key，速汇宝公钥，每个商家对应一个固定的速汇宝公钥（不是使用工具生成的商户公钥merchant_public_key，不要混淆），
			     即为速汇宝商家后台"支付管理"->"公钥管理"->"速汇宝公钥"里的绿色字符串内容
			2)demo提供的dinpay_public_key是测试商户号1111110166的速汇宝公钥，请自行复制对应商户号的速汇宝公钥进行调整和替换	*/		
			...
			result = RSAWithSoftware.validateSignByPublicKey(signInfo, dinpay_public_key, dinpaySign);	// 验签   signInfo速汇宝返回的签名参数排序， dinpay_public_key速汇宝公钥， dinpaySign速汇宝返回的签名
		}
		...
		if("RSA".equals(sign_type)){ // 数字证书加密方式 sign_type = "RSA"
			...
			result = mh.validateSignByPubKey(merchant_code, signInfo, dinpaySign);						// 验签    merchant_code为商户号， signInfo速汇宝返回的签名参数排序， dinpaySign速汇宝返回的签名
		}
