...
{   
	...
	//生成下注内容
		//注单数获取
	$betTypeId = getArrayValue("betTypeId", "", $data);
	$betOn = getArrayValue("betOn", "", $data);
	$betType = getArrayValue("betType", "", $data);
	$odds = getArrayValue("odds", "0", $data);
		//注单数据获取end
	$contentStr = getArrayValue($BetTypeID, "", $GLOBALS["IGSSCBetTypeName"]);
	$BtTypeFit = getArrayValue($BetTypeID, "", $GLOBALS["IGSSCBetTypeFit"]);
	$tBtConfigDetail = getArrayValue($BtTypeFit, array(), $GLOBALS["IGSSCBetDetail"]);
	$tBetOnConfig = getArrayValue($betOn, array(), $tBtConfigDetail["betOn"];
	$tBetTypeConfig = getArrayValue($betType, array(), $tBtConfigDetail["betType"];
	if ((!empty($tBetOnConfig))&&(!empty($tBetTypeConfig)) {
		$contentStr = $contentStr."-".getArrayValue($betOn, "",tBetOnConfig).chr(10);
		if substr($betType,0,3) == "NO" {						//"NO_"开头只显示数值
			$contentStr = $contentStr.substr($betType,3,strlen($betType))."号";
		} else {
			if $betOn == "SERIAL" {								//连号则直接显示连号内容：该类型的字符串的第一组数值为真实下注行为。其他皆为複式
				$contentStr = $contentStr.getArrayValue("betDetails", "", $data);
			} else {
				$contentStr =$contentStr.getArrayValue($betType, "", $tBetTypeConfig);
			}
		}
		$contentStr = $contentStr."@".$odds;
	} else {
		$contentStr = "todo:IGSSC need more config:".betOn.":".$betType;
	}
	//生成下注内容end
    if (!empty($recordTime)) {
