...
function makeIBCContentStr($data)
{
    ...
    if ($ParlayRefNo == 0) {
        ...
        $BtConfig = getArrayValue($BetTypeID, array(), $GLOBALS["betTypeName"]);
  		...
  		$BetTag = getArrayValue("Bet_tag", "", $content);
		$BetTypeStr = makeIBCbetType($BetTypeID,$BetTag);		//生成下注内容
		//-------------下注明细
		$BetArgus = getArrayValue("argus", array(), $BtConfig);
		if (!empty($BetArgus) && !empty($BetTeam) && !empty($OddsType)) {
			$BetDetailType = (getArrayValue("detail", array(), $BtConfig);
			if (!empty($BetDetailType){
				if $BetDetailType =="GetTeamName" {
					$btTeamStr = $BetTeam;								//调用沙巴接口获取队伍名(todo)
				} elseif $BetDetailType =="KeyInfo" {
					$btTeamStr = $BetTeam;								//直接显示下注对象，不转换
				}
			} else {
				$btTeamStr = getArrayValue($BetTeam, "", $BetArgus);		//这里
			}
        ...
        foreach ($parleyData as $_parley) {
            ...
            $BtConfig = getArrayValue($BetTypeID, array(), $GLOBALS["betTypeName"]);
			$BetTag = getArrayValue("Parlay_Bet_tag", "", $_parley);
			$BetTypeStr = makeIBCbetType($BetTypeID,$BetTag)		//生成下注内容
            $BetArgus = getArrayValue("argus", array(), $BtConfig);
 ...
 * 构建沙巴下注内容
 *
 * @param [type] $BtConfig    	配置数据
 * 
 * @return $BetTypeStr			返回下注内容
 */
 function makeIBCbetType($BetTypeID,$BetTag)
{
    $BtConfig = getArrayValue($BetTypeID, array(), $GLOBALS["betTypeName"]);
	if (!empty($BtConfig)) {
		   $BetTypeStr = $BtConfig["name"];
		if $BtConfig["title"] == "ReplaceX" {							//如果特殊则进行处理
			$BetTypeStr = str_replace("X",$BetTag,$BetTypeStr);				//从数据中获取X的值
		}				
		if $BtConfig["title"] == "ReplaceXY" { 							//如果特殊则进行处理
			$BetTypeStr = str_replace("X",substr($BetTag,0,2),$BetTypeStr);				//从数据中获取X的值
			$BetTypeStr = str_replace("Y",substr($BetTag,3,2),$BetTypeStr);				//从数据中获取Y的值
		}				
	}
	else {
		   $BetTypeStr = "todo:need more configs:".$BetTypeID;				//如果没有配置，则显示该ID需要检测配置
	}
	return $BetTypeStr;
}
/**
